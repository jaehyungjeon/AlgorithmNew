...
/*
	Question : 파티가 끝나고 나면, 사람들은 누가 파티에 왔는지와 얼마나 많은 사람들이 왔는지를 궁금해한다. 
	...
	Solution : 1. Tokenizer로 쪼개진 값에 standard값을 차감해주면 됨.
*/
...
public class BaekJoon_2845 {
	...
	public static void main(String[] args) throws Exception {
		...
		Integer person = Integer.parseInt(st.nextToken());
		Integer area = Integer.parseInt(st.nextToken());

		Integer standard = person * area;
		st = new StringTokenizer(br.readLine());
		for(int i=0; i<5; i++) {
			sb.append(Integer.parseInt(st.nextToken()) - standard).append(" ");
		}
